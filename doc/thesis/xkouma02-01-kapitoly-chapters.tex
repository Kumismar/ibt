...
\begin{definition}\label{def_bkg}
    ...
    \end{itemize}
\end{definition}
\dotaz{Patří derivační krok sem nebo až k~syntaktické analýze?}
...
\section{\dotaz{Co dalšího doplnit?}}
...
Prvním a~nejpřirozenějším příkladem je režim $*$ \dotaz{Jak tento režim nazvat?}, který byl v~\ref{def_derivacni_krok} definován pro bezkontextovou gramatiku a~princip v~CD gramatických systémech je podobný.
...
\begin{example}
    ...
    \dotaz{Nejsem si jistý, jestli takovýto styl psaní příkladů je v pořádku.}
\end{example}
...
\end{definition}
\dotaz{Nějaké tipy a triky pro velmi nevhodné přetečení/zalamování řádků za nevhodným symbolem? $v_k \in F(\ldots)$ bylo na dvou řádcích, musel jsem manunálně zalomit za \uv{že}, jinak by zalomení bylo za \uv{$\in$}.}
...
\begin{itemize}
    ...
    \begin{itemize}[label=$\circ$]
        ...
        \item \dotaz{Když je hybridní, může být obojí?
        $H$ explicitně specifikuje, že každá komponenta bude mít svůj derivační režim, ale nic neříká o (ne)de\-ter\-mi\-nis\-mu, pokud to správně chápu.
        Jestli je to tak, moc mi nedává smysl, aby se $H$ řadilo ke specifikaci determinismu, když specifikuje něco jiného.}
    \end{itemize}
\end{itemize}
