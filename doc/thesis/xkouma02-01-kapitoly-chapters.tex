...
Každé pole precedenční tabulky má hodnotu jednoho ze symbolů množiny $\{<, >, =, \times\}$.
    ...
    \begin{algorithmic}[1]
        ...
        \Repeat
            \Switch{Tabulka[\texttt{topmost\textunderscore token, input\textunderscore token}]}
                ...
                \EndCase
            \EndSwitch
        \Until{$b = \$$ \textbf{and} \texttt{stack.top()} = $\$S$}
    \end{algorithmic}
\end{algorithm}
...
Pokud je v~políčku tabulky symbol $=$ nebo $<$, pak analyzátor provede operaci $SHIFT$, přičemž pro symbol $<$ na zásobník prvně vloží $<$ a~až poté vstupní symbol.
Symbol $<$ slouží k~rozpoznání řetězce, který má být v~rámci redukce na zásobníku nahrazen.
Pokud je v~tabule na políčku $>$, pak proběhne operace $REDUCE$, při které:
\begin{enumerate}
...
\begin{example}
    ...
    \begin{table}[h]
        \centering
        \begin{tabularx}{0.33\textwidth}{X|XXXXXX}
            \hline 
            & $\wedge$ & $\vee$ & $i$ & $($ & $)$ & \$ \\
            \hline
            $\wedge$ & $>$ & $>$ & $<$ & $<$ & $>$ & $>$ \\
