...
\begin{figure}[h]
    \centering
    \includesvg{obrazky-figures/derivacni_krok_bkg}
...
\begin{definition}\label{def_konecny_automat}
    ...
    \begin{itemize}
        ...
        \begin{itemize}[label=$\circ$]
            ...
            \item Další možnost zápisu je zobrazení $R: Q \times \Sigma \rightarrow 2^Q$ \cite{TIN-opora}.
        \end{itemize}
...
\begin{definition}\label{def_prechod_ka}
    ...
    Symboly $\vdash_{\scriptscriptstyle M}^n, \vdash_{\scriptscriptstyle M}^+$ a~$\vdash_{\scriptscriptstyle M}^*$ nechť značí příslušně $n$-tou mocninu pro $n \geq 0$, tranzitivní a reflexivně-tranzitivní uzávěr relace $\vdash_M$ podobně jako u~derivačního kroku BKG (\ref{def_derivacni_krok}), čímž reprezentují \emph{sekvenci přechodů} KA $M$.
...
\begin{definition}\label{def_zasobnikovy_automat}
    ...
    \begin{itemize}
        ...
        \begin{itemize}[label=$\circ$]
            \item každé pravidlo $(A, q, a, w, p)$ může být zapsáno ve tvaru $Aqa \rightarrow wp$, kde $A \in \Gamma,$ $q, p \in Q,\; a \in \Sigma \cup \{\varepsilon\},\; w \in \Gamma^*$,
        \end{itemize}  
...
\begin{definition}
    ...
    \begin{equation*}
        (w, q, \alpha) \in \Sigma^* \times Q \times \Gamma^*, 
    \end{equation*}
    kde
    \begin{itemize}
...
\begin{definition}
    ...
    Relace $\vdash_{\scriptscriptstyle M}^n,\; \vdash_{\scriptscriptstyle M}^+,\; \vdash_{\scriptscriptstyle M}^*$ jsou opět $n$-tou mocninou relace, tranzitivním a~reflexivně-tran\-zi\-tiv\-ním uzávěrem.
\end{definition}
