...
U~obou jazyků záleží na velikosti písmen (jsou tedy \emph{case-sensitive}).
...
Veškeré proměnné jsou lokální, i~v~rámci hlavního těla programu. \todo{tohle napsat do prvni podkapitoly}

Jazyk Koubp je narozdíl od jazyka IFJ22 staticky typovaný (datový typ proměnné se určuje za překladu).
Dále nemá povinný \emph{prolog}, který je v~jazyce IFJ22 uveden pro kompatibilitu s~PHP.

\section{Obecné vlastnosti a~datové typy}
...
\begin{itemize}
    \item Definice a přiřazení
    \item Větvení
    \item Cyklus while a cyklus for 
    \item Příkaz return
\end{itemize}
