...
\dotaz{Tento dotaz je vlastně na všechny kapitoly - snažil jsem se občas něco vymyslet, občas něco opsat z literatury, ale pořád vůbec nemám tušení, jak kapitoly uvádět.
Přijde mi to takové mdlé.}
...
\todo{jendoduchy priklad KA i s prechodovym diagramem}
...
\begin{example}\label{example_za}
    ...
    \end{alignat*}
    \todo{Jestli bude cas - pridat prechodovy diagram}
\end{example}
...
\todo{v cele kapitole refaktorovat pismenka dle konvence}
...
\begin{algorithm}[h]
    ...
    \todo{pouzit Input Output a NewLine}
    \begin{algorithmic}[1]
...
\dotaz{ciste typograficky, vadi zalomene rovnice?}
...
\begin{algorithm}[h]
    ...
    \begin{algorithmic}[1]
        ...
        \EndFor
        \ForAll{$A \rightarrow \alpha \in R$}
...
\begin{definition}\label{def_mnozina_first}
    ...
    \end{equation*} 
    \todo{ilustrujici obrazek (podobny jako v ifj prezentaci 7)}
\end{definition}
\begin{algorithm}[h]
...
\begin{center}
    \texttt{if, while, for, return, elseif, else, function, if, float, string, bool}.
\end{center}

Identifikátory proměnných jsou posloupnosti alfanumerických znaků a~znak \textunderscore; identifikátor nesmí začínat číslicí.
...
Jazyk Koubp nepodporuje \emph{přetěžování (overloading)}\footnote{\todo{vlozit odkaz na definici overloadingu}} funkcí.
...
\begin{center}
    \todo{doplnit definici funkce treba faktorial nebo neco jednoduchyho}
\end{center}

\subsection*{Příkazy}
Jazyk Koubp podoruje obvyklé příkazy a~konstrukce programovacích jazyků.
\begin{itemize}
    ...
    \end{align*}
    \item \emph{Větvení}
    \item \emph{Cyklus while}
    \item \emph{Cyklus for}
    \item \emph{Volání funkcí}
\end{itemize}
...
\dotaz{samotne gramatiky patri do prilohy nebo je mam hodit sem?}
...
\begin{figure}[ht]\label{fig_schema_analyzy}
    \caption{Schéma analýzy}
    \todo{Tady bude obrázek znázorňující oba dva parsery a šest gramatik a který s~čím pracuje.}
\end{figure}
...
\begin{figure}\label{fig_schema_zmeny_parseru}
    \caption{Výtažek z kódu}
    \todo{tady bude vytazek z~kodu pro prepinani analyzatoru.}
\end{figure}
...
\begin{figure}[h]\label{fig_fcall_stridani}
    \caption{Obrazek volani funkce a stridani analyzatoru}
    \todo{Tady bude obrazek, kde bude napsany jednoduchy kod volani funkce, jeho reprezentace v~tokenech a navic vlozene pomocne tokeny, ktere parsery vyuzivaji.
